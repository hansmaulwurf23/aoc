...
def bfs_max(rooms, root, min_steps):
    q = deque()
    q.append((root, 0))
    visited = set()
    found = set()
...
def build_rooms(input, cur_pos):
    ...
    while input:
        ...
        if nxt in dirs:
            ...
            # if alternatives contain empty parts, we only run the detours separately and not the whole rest
